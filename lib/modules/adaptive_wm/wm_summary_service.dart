...
class WmSummaryService {
  static pw.Document buildDocument({
    ...
    final incorrectCount = records
        .where(
          (r) => r.accuracy == 0 && r.userResponse != MatchDecision.noResponse,
        )
        .length;
    final omissionCount = records
        .where((r) => r.userResponse == MatchDecision.noResponse)
        .length;
    final overallAcc = totalTrials == 0
        ? 0.0
        : (correctCount / totalTrials) * 100.0;

    final validRts = records
        .where((r) => r.reactionTimeMs != null)
        .map((r) => r.reactionTimeMs!)
        .toList();
    final meanRt = validRts.isEmpty
        ? 0.0
        : validRts.reduce((a, b) => a + b) / validRts.length;

    final maxSetSize = records.isEmpty
        ? 2
        : records.map((r) => r.setSize).reduce((a, b) => a > b ? a : b);
    ...
    // Condition stats
    final leftStats = _computeStat(
      records.where((r) => r.cuedHemifield == Hemifield.left).toList(),
    );
    final rightStats = _computeStat(
      records.where((r) => r.cuedHemifield == Hemifield.right).toList(),
    );
    final matchStats = _computeStat(
      records.where((r) => r.isMatchTrial).toList(),
    );
    final mismatchStats = _computeStat(
      records.where((r) => !r.isMatchTrial).toList(),
    );
  ...
  static Future<String> generate({
    ...
    final filePath =
        '${dataDir.path}/adaptive_wm_${cleanSubject}_summary_$stamp.pdf';
    ...
    return pw.Container(
      ...
      child: pw.Column(
        ...
        children: [
          ...
          pw.Text(
            'Paradigm: Visual Lateralized Adaptive N-Back (Color-Location)',
            style: pw.TextStyle(
  ...
  static pw.Widget _buildSetSizeTable(
    List<int> setSizes,
    Map<int, _ConditionStat> stats,
  ) {
    return pw.Column(
      crossAxisAlignment: pw.CrossAxisAlignment.start,
      children: [
        pw.Text(
          'PERFORMANCE BY N-BACK SET SIZE',
          style: pw.TextStyle(
            fontSize: 12,
            fontWeight: pw.FontWeight.bold,
            color: PdfColor.fromHex('#1E293B'),
  ...
  static pw.Widget _buildConditionTable(
    _ConditionStat left,
    _ConditionStat right,
    _ConditionStat match,
    _ConditionStat mismatch,
  ) {
    return pw.Column(
      crossAxisAlignment: pw.CrossAxisAlignment.start,
      children: [
        pw.Text(
          'PERFORMANCE BY TRIAL CONDITION',
          style: pw.TextStyle(
            fontSize: 12,
            fontWeight: pw.FontWeight.bold,
            color: PdfColor.fromHex('#1E293B'),
        ...
        pw.Table.fromTextArray(
          ...
          data: [
            [
              ...
              '${left.total}',
              '${left.correct}',
              '${left.accuracy.toStringAsFixed(1)}%',
              '${left.meanRt.toStringAsFixed(0)} ms',
            ],
            [
              'Right Hemifield Cue',
              '${right.total}',
              '${right.correct}',
              '${right.accuracy.toStringAsFixed(1)}%',
              '${right.meanRt.toStringAsFixed(0)} ms',
            ],
            [
              'Match Trial',
              '${match.total}',
              '${match.correct}',
              '${match.accuracy.toStringAsFixed(1)}%',
              '${match.meanRt.toStringAsFixed(0)} ms',
            ],
            [
              'Mismatch Trial',
              '${mismatch.total}',
              '${mismatch.correct}',
              '${mismatch.accuracy.toStringAsFixed(1)}%',
              '${mismatch.meanRt.toStringAsFixed(0)} ms',
            ],
          ],
        ),
      ],
  ...
  static List<pw.Widget> _buildTrialLogTable(List<TrialRecord> records) {
    // Return the table directly to MultiPage. Wrapping it in a Column makes
    // the entire full-length trial log indivisible and it cannot flow onto a
    // second page.
    return [
      pw.Text(
        'TRIAL-BY-TRIAL CHRONOLOGICAL LOG',
        style: pw.TextStyle(
          fontSize: 12,
          fontWeight: pw.FontWeight.bold,
          color: PdfColor.fromHex('#1E293B'),
